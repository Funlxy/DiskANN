...
int generate_pq_pivots_mpi(const float *const passed_train_data, size_t num_train, uint32_t dim, uint32_t num_centers,
                       ...
                       bool make_zero_mean)
{
    ...
    if (rank == 0) {
        uint32_t offsets_size = chunk_offsets.size();
        MPI_Bcast(&offsets_size, 1, MPI_UINT32_T, 0, MPI_COMM_WORLD);
